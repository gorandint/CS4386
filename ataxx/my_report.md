...
The main limitation of the current model is that it still relies on hand-crafted heuristics, which may not capture all the complexities of the game. Future work could explore the use of reinforcement learning to automatically learn heuristics from self-play, which has been successful in other board games like Go and Chess. That being said, the current heuristic-based model is still a competitive choice with limited time and computational resources. Secondly, according to related papers on Ataxx, "Performance of Monte Carlo Tree Search Algorithms when Playing the Game Ataxx" [2] and "Generalized Proof-Number Monte-Carlo Tree Search" [3], MCTS can perform well in Ataxx when tuned properly, so further improvements can be made by optimizing the MCTS implementation and its heuristics. Additionally, phase-aware heuristics and better aspiration window techniques can be explored to further enhance the search efficiency and performance of the agent.
